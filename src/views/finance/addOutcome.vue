...
<template>
  ...
  >
    <el-row>
      <el-col :span="8">
        <el-form-item label="支出原因" prop="outComeReason" style="width:100%">
      ...
      <el-col :span="8">
        <el-form-item label="支出金额" prop="amount" style="width:100%">
          <el-input-number style="width:100%" v-model="condition.amount" :min="0" />
        </el-form-item>
      </el-col>
      <el-col :span="8">
        <el-form-item label="支出时间" prop="outcomeDate" style="width:100%">
          <el-date-picker
            style="width:100%"
      ...
      <el-col :span="8">
        <el-form-item label="发票号码" prop="outcomeNumber" style="width:100%">
          <el-input v-model="condition.outcomeNumber"></el-input>
        </el-form-item>
      </el-col>
      <el-col :span="24">
...
export default defineComponent({
  ...
  setup() {
    ...
    const state: any = reactive({
      ...
      total: 0,
      rules: {
        outComeReason: [{required: true, message: '支出原因不能为空', trigger: 'blur'}],
        amount: [{required: true, message: '支出金额不能为空', trigger: 'blur'}],
        outcomeDate: [{required: true, message: '支出时间不能为空', trigger: ['blur', 'change']}],
      },
    })
    const onSubmit = () => {
      (conditionRef.value as any).validate((valid: boolean) => {
        if (valid) {
          if (state.condition.createdAt) {
            Outcome.update(state.condition, {
              where: {
                uid: state.condition.uid
              },
            }).then(() => {
              ElNotification.success('支出编辑成功')
              goCase()
            }).catch(() => {
              ElNotification.error('支出编辑失败!')
            })
          } else {
            ...
            Outcome.create(createDefault, {
              raw: true
            }).then(() => {
              ElNotification.success('支出新增成功')
              goCase()
            }).catch(() => {
              ElNotification.error('支出新增失败!')
            })
          }
        }
      })
